...
static void	Init_Lung(SoftDemo* pdemo)
{

	ConvexDecomposition::WavefrontObj wo;
	unsigned int tcount = 0;
	counttest = 0;
	//tcount = wo.loadObj("model.obj");
	...
	int *Indices;
	float *Vertices;
	TriCount = source.faces.size();
	Indices = new int[3 * source.faces.size()];
	Vertices = new float[3 * source.vertices.size()];
	for (int k = 0; k < TriCount; k++)
	{
		Indices[k * 3] = (int)source.faces.at(k).at(0) - 1;
		Indices[k * 3 + 1] = (int)source.faces.at(k).at(3) - 1;
		Indices[k * 3 + 2] = (int)source.faces.at(k).at(6) - 1;

	}

	for (unsigned int k = 0; k < source.vertices.size(); k++)
	{
		Vertices[k * 3] = (float)source.vertices.at(k).at(0);
		Vertices[k * 3 + 1] = (float)source.vertices.at(k).at(1);
		Vertices[k * 3 + 2] = (float)source.vertices.at(k).at(2);
	}
	...
	if (1){
		...
		//TRACEDEMO
		//btSoftBody*	psb = btSoftBodyHelpers::CreateFromTriMesh(pdemo->m_softBodyWorldInfo, wo.mVertices, wo.mIndices, wo.mTriCount);
		btSoftBody*	psb = btSoftBodyHelpers::CreateFromTriMesh(pdemo->m_softBodyWorldInfo, Vertices, Indices, TriCount);
		psb->generateBendingConstraints(2);
		psb->m_cfg.piterations = 2;
		psb->randomizeConstraints();
		//btMatrix3x3	m;
		...
		int forceSize = APPLIEDFORCEONGLOTTIS;
		int ind;
		Vector3f thePoint;
		Vector3f theCenter;
		Vector3f X;
		Vector3f direction;
		Vector3f perpdirection;
		int factor = 1;
		theCenter[0] = Glot.glottisCenterOfMass.at(0);
		theCenter[1] = Glot.glottisCenterOfMass.at(1);
		theCenter[2] = Glot.glottisCenterOfMass.at(2);
		//direction[0] = Glot.glottisEigevectors.at(2).at(0);
		//direction[1] = Glot.glottisEigevectors.at(2).at(1);
		//direction[2] = Glot.glottisEigevectors.at(2).at(2);
		perpdirection[0] = Glot.glottisEigevectors.at(1).at(0);
		perpdirection[1] = Glot.glottisEigevectors.at(1).at(1);
		perpdirection[2] = Glot.glottisEigevectors.at(1).at(2);

		for (unsigned int i = 0; i < Glot.pressurePointsIndex.size(); i++){
			ind = Glot.pressurePointsIndex.at(i);
			thePoint[0] = source.vertices.at(ind).at(0);
			thePoint[1] = source.vertices.at(ind).at(1);
			thePoint[3] = source.vertices.at(ind).at(2);

			X = theCenter + ((thePoint - theCenter).dot(perpdirection))*perpdirection;

			if ((X - theCenter).dot(perpdirection) > 0){ factor = -1; }
			else{ factor = 1; }

			psb->addForce(factor*forceSize * btVector3(Glot.glottisEigevectors.at(1).at(0), Glot.glottisEigevectors.at(1).at(1), Glot.glottisEigevectors.at(1).at(2)), Glot.pressurePointsIndex.at(i));
		}
...
void	SoftDemo::renderme()
{
	...
	for (int ib = 0; ib < sbs.size(); ++ib)
	{
		...
		for (int i = 0; i < psb->m_nodes.size(); ++i)
		{
			...
			/***Nousias********************************************/
			source.vertices.at(i).at(0) = psb->m_nodes[i].m_x[0];
			source.vertices.at(i).at(1) = psb->m_nodes[i].m_x[1];
			source.vertices.at(i).at(2) = psb->m_nodes[i].m_x[2];

		}
	}
	/***Nousias********************************************/
	counttest++;
	if ((counttest <= timeToModelExtraction) && (counttest>0)){

		if (counttest % 200 == 0){
			source.smoothing("taubin", "onoff", 150, 0.2,0.15);
			//source.exportToFile("model");
			parentObj.getPositionsFromChild(source);
			parentObj.exportToFile("model-" + std::to_string(counttest));	//object oriented implementation needed
			//system("pause");
			std::cout << "model-" + std::to_string(counttest) << std::endl;
		}
	}
	else
	{
...
int softBodyDeformPreprocess(){
	int argc = 1;
	char *argv[1];
	argv[0] = _strdup("DeformSim");
	SoftDemo* softDemo = new SoftDemo();
	softDemo->initPhysics();
	softDemo->getDynamicsWorld()->setDebugDrawer(&gDebugDrawer);
	glutmain(argc, argv, 1024, 768, "Soft body deformation", softDemo);
...
int softBodyDeform(glottis _glot, dotObj _source, dotObj _parent)
{
