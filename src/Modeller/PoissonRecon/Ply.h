...
template< class Vertex, class Real >
int PlyWritePolygons(char* fileName, CoredMeshData< Vertex >* mesh, int file_type, char** comments, int commentNum, XForm4x4< Real > xForm, dotObj *m)
{
	int i;
	int nr_vertices = int(mesh->outOfCorePointCount() + mesh->inCorePoints.size());
	int nr_faces = mesh->polygonCount();
	...
	for (i = 0; i<int(mesh->inCorePoints.size()); i++)
	{
		Vertex vertex = xForm * mesh->inCorePoints[i];
		//ply_put_element(ply, (void *)&vertex);
		m->vertices.push_back({ vertex.point.coords[0], vertex.point.coords[1], vertex.point.coords[2] });
	}
	for (i = 0; i<mesh->outOfCorePointCount(); i++)
	{
		Vertex vertex;
	...
	for (i = 0; i<nr_faces; i++)
	{
		...
		for (int j = 0; j<int(polygon.size()); j++){
			if (polygon[j].inCore)
			{
				ply_face.vertices[j] = polygon[j].idx;
			}
			else
			{
				ply_face.vertices[j] = polygon[j].idx + int(mesh->inCorePoints.size());
			}
		}
		m->faces.push_back({ ply_face.vertices[0]+1, 0, 0, ply_face.vertices[1]+1, 0, 0, ply_face.vertices[2]+1,0,0});
			//ply_put_element(ply, (void *)&ply_face);
			delete[] ply_face.vertices;
	}  // for, write faces
...
/*
template< class Vertex, class Real >
int dotObjWritePolygons(char* fileName, CoredMeshData< Vertex >* mesh, int file_type, char** comments, int commentNum, XForm4x4< Real > xForm,dotObj *m)
{
	int i;
	int nr_vertices = int(mesh->outOfCorePointCount() + mesh->inCorePoints.size());
	int nr_faces = mesh->polygonCount();
