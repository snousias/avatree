...
#define DOCUMENT_ROOT "E:/Workspace/VisualStudio2013/Projects/lungMorpherv4/bin/src/EmbeddedWebServer/html/"
#define PORT "8089"
...
public:
		...
		handleGet(CivetServer *server, struct mg_connection *conn)
	{
		...
		*/
		std::string fff = DOCUMENT_ROOT;

		std::ifstream file(fff+"index.html");
		std::string temp;
		while (std::getline(file, temp)) {
			mg_printf(conn, temp.c_str());
		}
